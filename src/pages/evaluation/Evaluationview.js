...
export default function User(props) {
    ...
    const gotonext = async () => {
        ...
        setQuestionindex(questionindex + 1, nextdata());
        // alert(questionindex)
        return true
    ...
    const gotoprev = async () => {
        ...
        setQuestionindex(questionindex - 1, prevdata());
    ...
    const nextdata = async (index) => {
        // await  sleep(2000) 
        if (questionindex >= 0) {
            setQuestion(single[questionindex].question);
            setMedia(single[questionindex].media);
            setHint(allanswer[questionindex].hint);
            setMultipleanswer(multipleanswer[index].inputfields)
            setType(allanswer[questionindex].type);
            setMarks(allanswer[questionindex].marks);// alert(questionindex)
            setAnswer(allanswer[questionindex].answer);// alert(questionindex)
        ...
        if (allanswer.length == 0) {
            ...
            getSingleAnswer(id).then((res) => {
                ...
                if (res.status) {
                    ...
                    if (allanswer.length == 0)
                    setAllanswer(res.results[questionindex].inputfields);
                    console.log('getSingleAnswer data state', res.results);
                    console.log('JSON parse data state', JSON.parse(res.results[questionindex].inputfields));
                    setMultipleanswer(JSON.parse(res.results[questionindex].inputfields))
                    setHint(JSON.parse(res.results[questionindex].inputfields));
                    setType(res.results[questionindex].type);
                    setMarks(res.results[questionindex].marks);
                                                                    ...
                                                                    <div className='editev' >
                                                                        ...
                                                                        <div className='editpart'>
                                                                            <div >
                                                                                ...
                                                                                <Form.Control readOnly={true} type="text" value={row.hint} onChange={(e) => setHint(e.target.value)}    ></Form.Control>
                                                                            </div>
                                                                            <div className='edittype'>
                                                                                <Form.Group controlId="exampleForm.ControlInput1" >
                                                                                    <Form.Label><h5>Type</h5></Form.Label>
                                                                                    <Form.Control readOnly={true} type="text" value={row.type} onChange={(e) => setType(e.target.value)}    ></Form.Control>
...
onChange={event => handleChangeInput(inputField.id, event)} ></Form.Control> */}
                                                                                    <Form.Control readOnly={true} type="text" value={row.marks} onChange={(e) => setMarks(e.target.value)}  ></Form.Control>
                                                                        ...
                                                                        <div >
                                                                            <Form.Group className="mb-3 " controlId="exampleForm.ControlInput1">
                                                                                ...
                                                                                <Form.Control readOnly={true} type="text" name='Answer' value={row.answer} onChange={(e) => setAnswer(e.target.value)} placeholder='Answers' as="textarea" rows={5} ></Form.Control>
                                                                            </Form.Group>
